...
function Music() {
  const { data: playlists } = useFetch<Playlist[]>(() => api.get<Playlist[]>("/music/playlists?limit=100"));
  const { data: influencers } = useFetch<Influencer[]>(() => api.get<Influencer[]>("/music/influencers?limit=100"));
  ...
  return (
    <div className="space-y-8">
      ...
      <div>
        ...
        <div className="card overflow-x-auto">
          <table className="w-full">
            <thead><tr><th className="th">Match</th><th className="th">Playlist</th><th className="th">Genre</th><th className="th">Curator</th><th className="th">Followers</th><th className="th">Pitch</th></tr></thead>
            <tbody>
              {(playlists || []).map((p) => (
      ...
      <div>
        ...
        <div className="card overflow-x-auto">
          <table className="w-full">
            <thead><tr><th className="th">Name</th><th className="th">Niche</th><th className="th">Handle</th><th className="th">Followers</th><th className="th">Outreach</th></tr></thead>
            <tbody>
              {(influencers || []).map((i) => (
