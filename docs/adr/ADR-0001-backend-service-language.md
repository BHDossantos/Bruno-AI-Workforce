...
- **Identifier:** ADR-0001
- **Status:** **proposed** — awaiting decision by the accountable owner
...
This ADR presents the analysis and does not select an option. Per §16.3, a
significant deviation from the specification requires an accepted ADR; per §10.4,
a stack change must account for operational cost, security, portability, hiring
and migration impact. Those five axes structure the comparison below.
...
**Not made.** Three options are presented; the accountable owner selects one and
this ADR is updated to `accepted` with the rationale.

Scope of whichever decision is taken: the language for `services/` and
`connectors/`. Out of scope: `apps/web` stays TypeScript; `ai-gateway` and the
evaluation stack stay Python. Neither is in question.
...
**If Option A is confirmed** — nothing changes in the spec; this ADR records
*why*, which is the point. Standing obligations: codegen from the OpenAPI 3.1 /
protobuf contracts into Go must exist from the first service (retrofitting it
after several services diverge is the expensive path), and §16.6 coding standards
must cover all three languages before Gate A rather than after.

**If Option B or C is confirmed** — §10.4, §16.1's tree, §16.6 and §18.3's hiring
...
**Validation before accepting.** Build one thin vertical slice — the PagerDuty
